...
'
                        ...
                        mov next_line, #0
                        wrword next_line, request_scanline
...
'                        wrbyte  visible,vblank_signal
                        
                        mov     x,#1                    'do visible back porch lines
                        call    #blank_lines
...
:vert   if_z            xor     interlace,#1            'interlace skip?
        if_z            tjz     interlace,#:skip
                        ...
                        wrword next_line, request_scanline  ' Send request for this scanline
                        add next_line, #1
        ...
        if_nc           jmp     #:line}
{
        ...
        if_nz_and_c     add     x,#1
                        ...
                        wrword next_line, request_scanline ' Send 'end of screen' with scanline 192

{        if_nz           wrlong  invisible,par           'unless interlace and field1, set status to invisible
        if_nz           wrbyte  invisible,vblank_signal
        if_nz           rdbyte  t2,PPUstatus
        if_nz           or      t2,#$80
        if_nz           wrbyte  t2,PPUstatus}

        if_z            jmp     #:skipstatusupdate
                        rdlong  t2,par
                        cmp     t2,invisible    wz
        if_z            jmp     #:skipstatusupdate
                        wrlong  invisible,par           'unless interlace and field1, set status to invisible
                        rdbyte  t2,PPUctrl
                        test    t2,#$80 wz
       if_nz            wrbyte  invisible,nmi_request
                        rdbyte  t2,PPUstatus
                        or      t2,#$80 wz              ' must not leave z flag set
                        wrbyte  t2,PPUstatus
:skipstatusupdate        
...
:off                    movi    ctrb,t2
        ...
        if_nc           or      t1,#%00001_000
                        ...
                        mov     hx,#10                  'compute horizontal metrics
                        shl     hx,#8
                        or      hx,#10
                        shl     hx,#4
                        ...
                        mov     t1,#12                  'compute vertical metrics
                        mov     t2,#1
                        call    #multiply
...
nmi_request             long    SCANLINE_BUFFER-17
PPUstatus               long    SCANLINE_BUFFER-14
PPUctrl                 long    SCANLINE_BUFFER-16
...
_pins                   long    %011_0000       'pins
_mode                   long    %0001           'mode
...
_ho                     long    0               'ho
_vo                     long    0               'vo
_broadcast              long    60_000_000      '_xinfreq<<4  'broadcast
